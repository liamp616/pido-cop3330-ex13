...
public class App {
    ...
    public static void main(String[] args) {
        ...
        int principal = myApp.readPrincipal();
        double interest = myApp.readInterest();
        int years = myApp.readYears();
        int compounded = myApp.readCompounded();
        myApp.output(principal, interest, years, compounded);
    }

    public void output(int principal, double interest, int years, int compounded) {
        //  A = P(1 + r/n)^(n*t)
        double rn = ((interest / 100.0) / compounded) + 1.0;
        double b = compounded * years;
        double a = Math.pow(rn, b);
        double investment = Math.round((principal * a) * 100.0) / 100.0;
        //  System.out.println(investment);
        System.out.println("$" + principal + " invested at " + interest + "% for " + years + " years compounded " + compounded +
                            " times per year is $" + investment + ".");
    }

    public int readCompounded() {
